...
class InicioViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate,UISearchBarDelegate {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        let tienda = UserDefaults.standard.string(forKey: "tt")!
        let categoria = UserDefaults.standard.string(forKey: "dd")!
        titulocat.text = "\(tienda)-\(categoria)"
    ...
    func consulta(){
        let pkcategoria =  UserDefaults.standard.string(forKey: "tpkcategoria")!
        let pktienda =  UserDefaults.standard.string(forKey: "pktienda")!
        
        let datos_a_enviar = ["PK_TIENDA": pktienda as Any,"PK_CATEGORIA": pkcategoria as Any] as NSMutableDictionary
        
        
        //ejecutamos la función arrayFromJson con los parámetros correspondientes (url archivo .php / datos a enviar)
        let dataJsonUrlClass = JsonClass()
        dataJsonUrlClass.arrayFromJson(url2:"ProductosByTiendaAndCategoria",datos_enviados:datos_a_enviar){ (datos_recibidos) in
    ...
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        ...
        self.bdesc.removeAll()
        self.bprecio.removeAll()
        self.burl.removeAll()
        self.burlstring.removeAll()
        
        var i = 0
        for item in self.titulo {
            
            if (item.lowercased().contains(searchBar.text!.lowercased())) {
                self.burlstring.append(self.url[i] )
                
                self.btitulo.append(item)
                self.bdesc.append(self.desc[i])
                self.bprecio.append(self.precio[i])
                self.bpkp.append(self.pkp[i])
            }
            i = i + 1
        }
